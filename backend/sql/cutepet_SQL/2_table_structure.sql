...
CREATE TABLE Giong(
    ...
    CONSTRAINT fk_Giong2Loai FOREIGN KEY(ma_loai) REFERENCES Loai(ma_loai) on UPDATE RESTRICT on DELETE RESTRICT
);
commit;
...
CREATE TABLE ThuCung (
    ...
    CONSTRAINT fk_thucung_giong FOREIGN KEY (ma_giong) REFERENCES Giong(ma_giong) on UPDATE RESTRICT on DELETE RESTRICT,
    CONSTRAINT fk_thucung_ngdung FOREIGN KEY (ma_nguoi_chu) REFERENCES NguoiDung(ma_nguoi_dung) on UPDATE RESTRICT on DELETE RESTRICT
);
commit;
...
CREATE TABLE Lich(
    ...
    ma_thu_cung int not null,
    PRIMARY KEY (ma_lich),
    CONSTRAINT fk_lich_theloailich FOREIGN KEY(ma_the_loai_lich) REFERENCES TheLoaiLich(ma_the_loai_lich) on UPDATE RESTRICT on DELETE RESTRICT,
