...
async function hasUserLikedListPosts_1(userId, listPostId) {
	...
	const postsHaveLiked =  await StatusPostModel.getLikeThePostInforOfListPosts(userId, listPostId).then((data) => data.payload);
...
async function insertUserLikePostInforOfListPosts(userId, listPosts){
	...
	listPosts.forEach((post, index) => {
		listPosts[index] = {...post, hasLiked: matching_table[post._id.toString()]};
	})
	return listPosts;
}
