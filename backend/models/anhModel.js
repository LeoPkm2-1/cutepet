...
const getAnhByURL = async (url, orderByTime = 0) => {
  ...
  else sqlStmt = "select * from Anh where url = ? ORDER BY ngay_cap_nhat DESC";
...
const getAnhByMaAnh = async (ma_anh, orderByTime = 0) => {
  ...
  if (orderByTime == 0) sqlStmt = "select * from Anh where ma_anh = ?";
    ...
    sqlStmt = "select * from Anh where ma_anh = ? ORDER BY ngay_cap_nhat ASC";
  else
    sqlStmt = "select * from Anh where ma_anh = ? ORDER BY ngay_cap_nhat DESC";
...
const deleteListAnhByListMaAnh = async (listMaAnh) => {
  const sqlStmt = `delete from Anh where ma_anh in (${listMaAnh.toString()})`;
  return await sqlQuery(sqlStmt)
    .then((data) => new Response(200, data, ""))
    .catch((err) => new Response(400, [], err.sqlMessage, err.errno, err.code));
};
