...
export default function PostShareServiceComponent(props: Props) {
  ...
  const [tenCuahang, setTenCuaHang] = useState('BKPET Care');
  ...
  useEffect(() => {
    if (status?.serviceInfo?.shopId)
      shopApi.getMyShop(status?.serviceInfo?.shopId).then((data) => {
        console.log(data, ' shop nè');
        setTenCuaHang(data?.payload?.ten || '');
      });
  }, [status?.serviceInfo?.shopId]);
      ...
      postApi.getStatusById(props.idStatus).then((data: any) => {
        if (data?.status == 200) {
          ...
          const sta: StatusType = {
            ...
            serviceInfo: {
              serviceName: data?.payload?.serviceInfor?.serviceName || '',
              shopId: data?.payload?.serviceInfor?.shopId || 0,
              serviceType: data?.payload?.serviceInfor?.serviceType || [],
              numOfStar: data?.payload?.serviceInfor?.numOfStar || 0,
              shortDescription:
                data?.payload?.serviceInfor?.shortDescription || '',
            },
          };
          setStatus(sta);
        }
  ...
  useEffect(() => {
    console.log('vào comment', status);
    ...
    if (status && status?.id) {
      ...
      postApi
        ...
        .then((data) => {
          if (data?.status == 200) {
            if (data?.payload?.comments?.length == 0) {
              setIsHasComment(false);
              setComments([]);
              return;
            }
    ...
    <>
      <>
        ...
        >
              ...
              >
                ...
                >
                  ...
                  <span
                    ...
                    onClick={() => {
                      navigate(`/home/cua-hang/${status?.serviceInfo?.shopId}`);
                    }}
                  >
                    {tenCuahang}
                  </span>
          ...
          <Box
            onClick ={() => navigate(`/home/cua-hang/${status?.serviceInfo?.shopId}/dich-vu/${status?.serviceId}`)}
            sx={{
              margin: '20px',
          ...
          >
            ...
            >
              ...
              >
                {status?.serviceInfo?.serviceName}
              </Typography>
              <Typography
                sx={{
                  fontFamily: 'quicksand',
              ...
              >
                {status?.serviceInfo?.shortDescription}
              </Typography>
              <Box
                sx={{
                  display: 'flex',
              ...
              >
                ...
                {status?.serviceInfo?.serviceType.map((item: string) => {
                  return <Tag text={item} />;
                })}
              </Box>
              <Box>
                <Rating
                  ...
                  value={status?.serviceInfo?.numOfStar || 0}
                  readOnly
                />
              </Box>
            </Box>
          ...
          >
            ...
            >
              ...
              >
                {isComment ? 'Ẩn bình luận' : 'Bình luận'}
              </Typography>
              <SmsOutlinedIcon
                sx={{
                  color: 'gray',
                  ...
                  return (
                    <Comment
                      ...
                      onRemove={() => {
                        ...
                        const listCmt: CommentType[] = deepCopy(comments);
                        listCmt.splice(index, 1);
                        setComments(listCmt);
                      }}
                    />
                  );
                  ...
                  replys.map((item, index) => {
                    return (
                      <Reply
                        ...
                        reply={item}
                        onRemove={() => {
                          const listReplys = deepCopy(replys);
                          listReplys.splice(index, 1);
                          setReplys(listReplys);
                        }}
                      />
                    );
                  })}
